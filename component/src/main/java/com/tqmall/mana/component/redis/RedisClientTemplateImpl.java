...
public class RedisClientTemplateImpl implements RedisClientTemplate {
    ...
    public Object runTask(Callback callback) {
        ...
        try {
            ...
            return callback.onTask(shardedJedis);
    ...
    private boolean handleJedisException(JedisException jedisException) {
        if (jedisException instanceof JedisConnectionException) {
            ...
            String message = jedisException.getMessage();
            if (message != null && message.toUpperCase().indexOf("READONLY") != -1) {
                log.error("Redis connection are read-only slave.", jedisException);
            } else {
                // dataException, isBroken=false
                log.error("unKnow error", jedisException);
                return false;
            }
        } else {
...
/*=========================*/
    ...
    public <T> T lazyGet(String key, Class<?> cls) {
        try {
            ...
            return (T) JsonUtil.strToObject(jsonString, cls);
    ...
    public <T> List<T> lazyGetList(String key, Class<?> cls) {
        ...
        try {
            ...
            return JsonUtil.strToList(jsonString, cls);
    ...
    public void setNone(String key) {
        setNone(key, RedisKeyBean.RREDIS_EXP_HOURS);
    }

    @Override
    public void setNone(String key, Integer expire) {
        setStringWithTime(key, RedisKeyBean.NULL_OBJECT, expire);
    }
...
//            String str= StringUtils.join(list.toArray(),",");
            ...
            result = shardedJedis.rpush(key, (String[]) list.toArray(new String[list.size()]));
