...
$(document).ready(function () {
    ...
    $('.div-left-ul .mana-table').off('click.aa').on('click.aa', '.js-delete', function (e) {
        ...
        $('.item-select').refresh();
        $(dom).remove();
        if ($('.div-left-ul .mana-table tr').length == 0) {
            $('.div-left-ul .item-box').parents("li").remove();
        }
        // addOption(e);
    });
...
//新增一条项目条目
function saveItem() {
    var itemName = $('.item-select').find('option:selected')[0].innerText;
    ...
    if (!isNaN(itemNum) && (/^\d+$/.test(itemNum))) {
        ...
        $('.mana-table').off('click.aa').on('click.aa', '.js-delete', function (e) {
            ...
            var id = $(dom).find(".js-item-name").data("id");
            var itemName = $(dom).find(".js-item-name")[0].innerText;
            var str = '<option value="' + id + '">' + itemName + '</option>';
            $('.item-select').prepend(str);
            $('.item-select').refresh();
            $(dom).remove();
    ...
    Ajax.get({
        ...
        success: function (result) {
            if (result.success) {
                ...
                var itemList = [];
                var domList = $('.mana-table').find("tr");
                ...
                for (var i = 0; i < itemList.length; i++) {
                    _h += '<option value="{0}">{1}</option>'.format(itemList[i].id, itemList[i].itemName);
                }
...
function initOption() {
    ...
    for (var i = 0; i < _itemList.length; i++) {
        _h += '<option value="{0}">{1}</option>'.format(_itemList[i].id, _itemList[i].itemName);
    }
    $('.item-select').html(_h);
    $('.item-select').chosen({
...
function sureBtn() {
    ...
    var marketTip = '';
    ue1.ready(function () {
        packageDescription = ue1.getPlainTxt();
    });
    ...
    Ajax.postJson({
        url: '/insurancePackage/updateServicePackageConfig',
        data: servicePackage,
        success: function (result) {
            layer.close(index);
            if (result.success) {
