/**
 * Created by zhouheng on 17/2/28.
 */
...
}).on('click', '.js-goods-delete', function (e) {
...
function saveVideo() {
    ...
    $('.js-down-ul .mana-table').append(template('videoTableTrTemplate', {
        videoName: videoName,
...
function sureBtn() {
    ...
    $.each(domList, function (index, dom) {
        ...
        if (isMatchModel == 1) {
            materialType = $dom.find('.js-goodsSn').data("type");
        } else {
            goodsSn = $dom.find('.js-goodsSn')[0].innerText;
        }
    ...
    Ajax.postJson({
        url: "/insuranceItem/updateServiceItemConfig",
        data: serviceItem,
        success: function (result) {
            layer.close(index);
            if (result.success) {
