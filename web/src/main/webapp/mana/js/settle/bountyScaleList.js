...
$(document).ready(function () {
    ...
    location.href = "/settle/shopRule/bountyScaleManage";
...
function clickEvent() {
    ...
    $(document).on('click','.remove-tr',function () {
        ...
        LayerUtil.confirm("确定要删除奖励金模式比例设置吗?",function () {
            Ajax.get({
...
function queryBountyScaleList(pageNumber) {
    ...
    var data  = settleCommon.getData(1);
    ...
    Ajax.get({
        ...
        success:function (result) {
            ...
            if(result.success){
                ...
                var body_html=tableList(list);
                $('.mana-body').html(body_html);
                ...
                var body_html_none = '<tr><td class="ver-align" colspan="12">暂无数据</td></tr>';
...
function tableList(list) {
    ...
    for (var i = 0; i<list.length;i++){
        ...
        body_html += '<td class="ver-align" rowspan="'+list[i].rowSpan+'">'+list[i].insuranceCompanyName+'</td>'+'<td class="ver-align" rowspan="'+list[i].rowSpan+'">'+list[i].settleConditionName+'</td>'+applyRangeHtml;
        for(var j=0;j<list[i].rebateStandardVOs.length;j++){
            ...
            for (var k = 0;k<StandardVOs.configBasicVOList.length;k++){
                ...
                var operate = '<td class="ver-align" rowspan="'+list[i].rowSpan+'">' +
                    '<a href="/settle/shopRule/bountyScaleManageEdit?bountyScaleId='+list[i].id+'" class="modify">修改</a>' +
